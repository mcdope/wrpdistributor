...
composer:
	docker-compose exec -T php_$(PROJECT_NAME) composer install
...
migrate_database:
	docker-compose exec -T php_$(PROJECT_NAME) ping mysql_wrpdistributor && ./vendor/bin/doctrine-migrations migrations:migrate --no-interaction -n -v
